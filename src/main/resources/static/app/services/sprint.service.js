"use strict";
angular.module('sprintGraphApp').factory(
		'SprintService',
		[ 'SprintResource', 'rx', 'StoryService',
				function(sprintResource, rx, storyService, BASE_URL) {
					...
					return {
						...
						getStories:function(sprint){
							return  rx.Observable.just(sprint).map(function(s){
								...
								return 	rx.Observable.fromPromise(sprintResource.getStories({id:id}).$promise);
							});
						}
					};
				} ]);